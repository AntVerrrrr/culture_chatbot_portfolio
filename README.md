...
> “코딩은 사용자와 상호작용하는 새로운 형태의 의사소통이다.”
> 
...
그 아이디어는 이후 친구가 **대통령 인재상**을 수상하는 계기가 되었고,

저는 그 비전을 실제 서비스로 구현하기 위한 기술 파트너로 합류했습니다.
...
| --- | --- | --- | --- |
| 🔧 1. 다국어 시스템
(i18n + modeltranslation) 충돌 해결 | • 정적 번역(i18n)과 DB 번역(modeltranslation)이 서로 다른 기준으로 작동
• URL 언어 프리픽스(/en/, /ja/)와 DB 필드 언어 불일치
• 일부 페이지에서 UI 언어와 DB 데이터 언어가 섞여 출력되는 문제 발생 | • 언어 프리픽스를 기준으로 DB 언어 필드를 자동 매핑하는 로직 구현
• i18n.js + Django i18n + modeltranslation을 하나의 언어 파이프라인으로 통합
• 캐시된 모델 데이터를 언어별로 분리해 반환하도록 API 구조 개선 | • UI와 DB 데이터가 5개국어에서 완전 동기화
• 언어 전환 시 페이지 전체가 즉시 갱신 |
| 🎤 2. Whisper STT + TTS 동시 사용 시 오디오 피드백 루프 문제 | • TTS가 출력한 음성이 STT 입력으로 다시 들어가는 피드백 루프 발생
• 녹음 중 음성 출력 시 중복 응답 또는 무한 응답 발생 | • `isSpeaking`, `isRecording` 상태 플래그 기반 상태 머신 설계
• STT 시작 시 TTS 음소거 처리
• debounce + request lock으로 race condition 제거 | •• 음성 기반 대화 안정화
• 중복 응답률 0% 달성 |
| 🔍 3. File Search(RAG) 품질 편차 및 불필요한 벡터 검색 문제 | • document_id 없는 Assistant도 RAG 쿼리가 실행되어 엉뚱한 문단 검색
• 유사 키워드가 많은 문화/전통주 데이터에서 매칭 정확도 흔들림 | • Assistant별 RAG ON/OFF 플래그 도입
• document_id 없을 때 벡터 검색 완전 비활성화
• “키워드 검색 + 임베딩 검색”의 하이브리드 구조로 품질 보정
• Responses API 프롬프트에 근거 문단 구조화하여 삽입 | • 근거 기반 답변 품질 향상
• 불필요한 벡터 검색 제거로 응답 성능 개선 |
| ⚙️ 4. 서버 배포(Nginx + Gunicorn) 안정화 과정 | • 초기 Gunicorn 서비스가 권한 문제로 반복 종료
• 정적 파일 경로 mismatch로 이미지 404 빈번
• CORS/ALLOWED_HOSTS 설정 오류로 API 호출 실패 발생
• 외부 MySQL 연결 시 설정 충돌 | • systemd 환경변수 정비 → Gunicorn 안정화
• Nginx 정적 파일 루트 경로 통일
• HTTPS(Certbot) 인증 & 자동갱신 설정
• MySQL 외부 커넥션 구조 분리 및 환경변수화 | • 1년 이상 무중단 운영
• 실제 트래픽 증가 상황에서도 안정성 유지 |
| 🗺️ 5. 지역 기반 동적 로딩(Province → City) 다국어 처리 문제 | • AJAX로 로드된 지역 데이터가 UI 언어와 불일치
• modeltranslation과 JS i18n 충돌 | • 지역 데이터를 locale 기반으로 자동 변환하는 middleware 구현
• 지역 캐싱 구조를 언어별로 분리 | • 지역 선택 UI 언어 일관성 100% |
| 💬 6. 대화 UI/UX 개선 
(자동 스크롤, 액션버튼, 프리뷰) | • 긴 응답에서 자동 스크롤 미작동
• 모바일 화면에서 버튼 UI가 콘텐츠를 가림
• 답변 후 액션 버튼(복사/좋아요/싫어요) 동적 생성 필요 | • IntersectionObserver 기반 자동 스크롤 구현
• 프리뷰 페이지 추천 구역 ON/OFF 기능 추가
• createActionButtons()로 동적 액션 버튼 생성 | • 자연스러운 사용자 경험 구현 |
